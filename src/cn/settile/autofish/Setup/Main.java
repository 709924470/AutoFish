...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		jb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
		        ...
		        if(!Main.isValidFolder(pth)) {
		        	JOptionPane.showMessageDialog(null, "不是有效的文件夹!", "警告", JOptionPane.ERROR_MESSAGE);
		        	return;
		        }
		        jta.setText(pth);
				jb1.setEnabled(true);
		...
		jb1 = new JButton("安装到这个版本");
		jb1.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				launcher l = Main.launcherType(jta.getText());
				//JOptionPane.showMessageDialog(null, "l="+l.name(),  "警告",JOptionPane.ERROR_MESSAGE);
				if(l == launcher.vanilla) {
					JOptionPane.showMessageDialog(null, "正在向原版(其他)MC启动器安装自动钓鱼MOD", "提示", JOptionPane.INFORMATION_MESSAGE);
					Main.install(jta.getText(), l);
				}else if(l == launcher.multimc) {
					JOptionPane.showMessageDialog(null, "正在向MultiMC启动器安装自动钓鱼MOD", "提示", JOptionPane.INFORMATION_MESSAGE);
					Main.install(jta.getText(), l);
				}else {
					JOptionPane.showMessageDialog(null, "不是有效的文件夹!",  "警告",JOptionPane.ERROR_MESSAGE);
				}
	...
	public static void install(String path, launcher l) {
		if(l == launcher.vanilla) {
			try {
				...
				fos.close();
				JOptionPane.showMessageDialog(null, "安装完成",  "提示",JOptionPane.INFORMATION_MESSAGE);
			} catch (Exception e) {
				e.printStackTrace();
				JOptionPane.showMessageDialog(null, "错误!\n" + e.getLocalizedMessage(),  "警告",JOptionPane.ERROR_MESSAGE);
			}
		}else if(l == launcher.multimc) {
			...
			try {
				...
				e.printStackTrace();
				JOptionPane.showMessageDialog(null, "错误!\n" + e.getStackTrace(),  "警告",JOptionPane.ERROR_MESSAGE);
			}
		}
	}
